...
export function getTooltipMarker(inOpt, extraCssText) {
  ...
  var color = opt.color;
  var opacity = String(opt.opacity || 1);
  var type = opt.type;
  ...
  if (renderMode === 'html') {
    ...
    // Only support string
    + encodeHTML(color) + ';' + 'opacity:' + encodeHTML(opacity) + ';' + (extraCssText || '') + '"></span>' : '<span style="display:inline-block;margin-right:4px;' + 'border-radius:10px;width:10px;height:10px;background-color:' + encodeHTML(color) + ';' + 'opacity:' + encodeHTML(opacity) + ';' + (extraCssText || '') + '"></span>';
  } else {
